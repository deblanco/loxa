...
describe('the welcome steps', () => {
  it('says what it is for, then asks for a photo, then explains the face', () => {
    // The order is the argument, so it is the thing worth pinning: nothing else
    // matters if they do not want the app, and the face explanation answers the
    // question the photo step raises.
    expect(WELCOME_STEPS).toEqual(['value', 'photo', 'face']);
  });

  it('walks forward to the end and stops', () => {
    expect(nextStep('value')).toBe('photo');
    expect(nextStep('photo')).toBe('face');
    expect(nextStep('face')).toBeNull();
  });
